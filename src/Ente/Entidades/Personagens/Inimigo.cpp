...
bool Inimigo::detectaJogador() {
    if (podeAndar) {
        ...
        if ((unsigned)(shapeJogador1.getPosition().x - corpo.getPosition().x) <= raio.x &&
        (unsigned)(shapeJogador1.getPosition().y - corpo.getPosition().y) <= raio.y) {
            alvo = jogador1;
        }
        if (jogador2 != NULL) {
            sf::RectangleShape shapeJogador2 = jogador2->getCorpo();
            if ((unsigned)(shapeJogador2.getPosition().x - corpo.getPosition().x) <= raio.x &&
            (unsigned)(shapeJogador2.getPosition().y - corpo.getPosition().y) <= raio.y) {
                if(alvo==NULL)
                    alvo= jogador2;
                else
...
void Inimigo::inimigoTomaDano(Jogador* jgdor) {
    if(jgdor!=NULL){
        if(jgdor->getAtacando()){
            ...
            if ((unsigned)(corpo.getPosition().x - posicaoJogador.x) <= rangeAtaque.x &&
                (unsigned)(corpo.getPosition().y - posicaoJogador.y) <= rangeAtaque.y){
