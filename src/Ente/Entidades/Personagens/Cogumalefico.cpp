...
sf::Vector2f Cogumalefico::tamanho=sf::Vector2f(200, 200);
sf::Vector2f Cogumalefico::velocidadeTerminal=sf::Vector2f(0.2, 0);
sf::Vector2f Cogumalefico::range=sf::Vector2f(2000, 2000);
...
void Entidades::Cogumalefico::inicializa() {
    ...
    animacao.addAnimacao("imagens/Cogumalefico/Morre.png", "MORRENDO", 4, 0.2f, sf::Vector2f(8,6));
...
void Entidades::Cogumalefico::atacaJogador(Entidades::Jogador *jogador) {
    if (!permiteAtacar) {
        ...
        if ((unsigned)(inimigo.x - jgdor.x) < raioAtaque.x && (unsigned)(inimigo.y - jgdor.y) < raioAtaque.y) {
            atacando = true;
            alvoAtaque = jogador;
        }
        if(atacando){
